                            ...
                            case 0 :
                                ...
                                if ( _domain ) {
                                    if ( _useCORS && error == 'No Transport' ) {
                                        ...
                                        console.log('CORS enabled... disabling');
                                        _useCORS = false;
                                        _refresh(channel, 0);
                                    }
                                    else if ( txt == 'timeout' ) {
						...
						$.ajax({
							...
							success: function(data, txt, xhr) {
								try {
									...
									var dsLen = (data.store && (typeof(data.store.length) != 'undefined')) ? data.store.length : -1;
									if ( dsLen >= 0 ) {
										_storeItems(data.store, _unread);
									}
									subscribe(_o);
								} catch(e) {
									/*FA.Debugger.log(data.store);*/
								}
							}
						});
					}
