...
function showMenu (event, user, me) {
	...
	$(div).append(	'<p class="cb-menu-title"> ' +
						((user.username.length > 9) ? user.username.substr(0,9)+'...' : user.username) +
						'<span title="Fermer la fenêtre" class="cb-menu-close" onclick="hideMenu()"> &times; </span>' +
					'</p>');
	
	/* --------- create the see profile link ------------- */	
	$(div).append("<p class='cb-menu-item'><a href='/u" + user.id + "' target='profile'>Voir le profil</a></p>");
	
	/* --------- create the send pm link ------------- */
    if(user.id != me.id) {
	    $(div).append("<p class='cb-menu-item'><a href='/privmsg?mode=post&u=" + user.id + "' target='profile'>Envoyer un MP</a></p>");
    }
	...
	if ($("#message").length && me.admin) {
		...
		if (user.chatLevel != 2 && !user.admin){
			/* -- Kick -- */
			$(div).append(	"<p class='cb-menu-item'>"+
								"<a href='#' onclick='return action_user(\"kick\", \"" + user.username + "\");'>Kicker du chat</a>"+
							"</p>");
			
			/* -- Ban -- */
			$(div).append(	"<p class='cb-menu-item'>"+
								"<a href='#' onclick='return action_user(\"ban\", \"" + user.username + "\");'>Bannir du chat</a>"+
							"</p>");			
		}
		...
		if (user.chatLevel == 2 && user.userLevel != 1 && user.id != me.id ) {
			...
			$(div).append(	"<p class='cb-menu-item'>"+
								"<a href='#' onclick='return action_user(\"unmod\", \"" + user.username + "\");'>Retirer modération</a>"+
							"</p>");				
		} else if (!user.admin) {
            // Add moderator privileges
            $(div).append(	"<p class='cb-menu-item'>"+
								"<a href='#' onclick='return action_user(\"mod\", \"" + user.username + "\");'>Ajout modérateur</a>"+
							"</p>");	           
		}
	}
	...
	if (connected && user.id == me.id) {
        $(div).append(	"<p class='cb-menu-item'>"+
                            "<a href='#' onclick='return action_user(\"away\", prompt(\"Raison\"));'>S\'absenter</a>"+
                        "</p>");	
        
        $(div).append(	"<p class='cb-menu-item'>"+
                            "<a href='#' onclick='chatbox.disconnect();hideMenu();'>Quitter</a>"+
                        "</p>");			
	}
...
function display_upload_servimg(button, account, id, f)
{
	...
	if ( !document.getElementById('obj_servimg') ) {
		container.innerHTML = '<p><iframe id="obj_servimg" src="http://www.servimg.com/multiupload.php?mode=fae&account=' + account + '&id=' + id + '&f=' + f + '" width="540" height="230" border="0" ></iframe></p>';
	}
...
function onMessage(e) {
    if(e.data.from === "servimg")
        $("#text_editor_textarea").sceditor('instance').insert(e.data.data + "\n");
}
