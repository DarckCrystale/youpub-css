...
// TODO: This code tag should be configurable
		...
		appendNewLine = function() {
            ...
            var lastNode = $wysiwygBody.children().last();
            var blockingNodes = {blockquote : 1, code : 1};
            if(lastNode.length){
                var lastNodeName = lastNode[0].nodeName.toLowerCase();
                if(lastNodeName == 'p'){
                    var lastChild = lastNode.children().last();
                        if(lastChild.length && lastChild[0].nodeName.toLowerCase() in blockingNodes)
                            lastNode.append("<br />");
                }
                else if(lastNodeName in blockingNodes){
                    $wysiwygBody.append("<br />");
                }
            }
		};
	...
	/** @lends jQuery.sceditor.dom */
	{
		 ...
		 * @type {string}
		 */
		blockLevelList: "|body|hr|p|div|h1|h2|h3|h4|h5|h6|address|pre|form|table|tbody|thead|tfoot|th|tr|td|li|ol|ul|blockquote|center|marquee|button|",
				...
				if(nodeType === 3 && nodeValue)
				{
					...
					if(isInline(node))
					{
						while((sibling = getSibling(sibling, true)))
						{
							...
							{
								// Patch FA : remove space in the regex because we need it.
