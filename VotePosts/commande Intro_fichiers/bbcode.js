			...
			while(toks.length > 0)
			{
				...
				else if(token.type === tokenType.open)
				{
					...
					if(token.attrs)
					{
						...
						{
							if(token.attrs.defaultattr.charAt(0) === "(" && token.attrs.defaultattr.charAt(token.attrs.defaultattr.length-1) === ")" && token.name != "quote") /*#1465 #1506 résolution mais pourquoi ce test ?*/
...
// TODO:This should check for CSS white-space, should pass it in the function to reduce css lookups which are SLOW!
	...
	$.sceditor.plugins.bbcode.bbcodes = {
		...
		// START_COMMAND: Quote CUSTOM BBCODE celui appelé est dans sceditor.custom-bbcode.js
		quote: {
			tags: {
				blockquote: null
			},
